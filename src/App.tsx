...
const isAutoGeneratedDefaultTitle = (title: string, date: number | Date) => {
  if (!title) return false;
  const trimmed = title.trim();
  if (trimmed === getSessionDefaultTitle(date, 'en') || trimmed === getSessionDefaultTitle(date, 'es')) {
    return true;
  }
  // Matches "Sat 12/09/26" or "Sab 12/09/26"
  const dayOfWeekPattern = /^(Mon|Tue|Wed|Thu|Fri|Sat|Sun|Lun|Mar|Mie|Mié|Jue|Vie|Sab|Sáb|Dom)\s+\d{2}\/\d{2}\/\d{2}$/i;
  if (dayOfWeekPattern.test(trimmed)) {
    return true;
  }
  // Matches legacy "Lesson ..." or "Clase ..."
  const legacyLessonPattern = /^(Lesson|Clase)\s+.+/i;
  if (legacyLessonPattern.test(trimmed)) {
    return true;
  }
  // Matches legacy "Fri, Feb 27, 2026"
  const legacyDatePattern = /^[A-Z][a-z]{2},\s+[A-Z][a-z]{2}\s+\d{1,2},\s+\d{4}$/;
  if (legacyDatePattern.test(trimmed)) {
    return true;
  }
  return false;
};
...
export default function App() {
  ...
  const [newSessionTargetDate, setNewSessionTargetDate] = useState<number | null>(null);
  ...
  // Translate auto-generated default session titles when app language changes or on load
  useEffect(() => {
    if (sessions.length === 0) return;

    let changed = false;
    const updatedSessions = sessions.map((session) => {
      if (isAutoGeneratedDefaultTitle(session.title, session.date)) {
        const expectedTitle = getSessionDefaultTitle(session.date, uiLanguage);
        if (session.title !== expectedTitle) {
          changed = true;
          const updated = { ...session, title: expectedTitle };
  ...
  const handleOpenNewSession = (targetDate?: Date | number) => {
    ...
    if (targetDate) {
      ...
      const midday = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 12, 0, 0).getTime();
      setNewSessionTargetDate(midday);
    } else {
      setNewSessionTargetDate(null);
    }
  ...
  const handleSelectEntryOption = async (option: EntryOption, file?: File) => {
    ...
    const sessionDate = newSessionTargetDate ?? Date.now();
    setNewSessionTargetDate(null);
    ...
    if (file) {
      if (option === 'video') {
        const newSession: Session = {
          id: crypto.randomUUID(),
          title: getSessionDefaultTitle(sessionDate, uiLanguage),
          subtitle: '',
          date: sessionDate,
...
function SessionDetail({
  ...
  const formatDateToDMY = (timestamp: number) => {
    const d = new Date(timestamp);
    const day = String(d.getDate()).padStart(2, '0');
    const month = String(d.getMonth() + 1).padStart(2, '0');
  ...
  const formatDMYToIso = (dmy: string) => {
  ...
  const [tempDate, setTempDate] = useState(() => formatDateToDMY(session.date));
  const dateInputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (!isSettingsOpen) {
      setTempActiveGlossaryIds(activeGlossaryIds);
      setTempDate(formatDateToDMY(session.date));
    }
  }, [activeGlossaryIds, session.date, isSettingsOpen]);
  ...
  const handleConfirmSettings = () => {
    ...
    let newDateTimestamp = session.date;
    let titleUpdate: Partial<Session> = {};

    if (tempDate) {
      let day = 0, month = 0, year = 0;
      if (tempDate.includes('/')) {
        const parts = tempDate.split('/').map(Number);
        day = parts[0];
        month = parts[1];
        year = parts[2];
      ...
      if (day >= 1 && day <= 31 && month >= 1 && month <= 12 && year >= 1900 && year <= 2100) {
        newDateTimestamp = new Date(year, month - 1, day, 12, 0, 0).getTime();
        if (isAutoGeneratedDefaultTitle(session.title, session.date)) {
          titleUpdate = { title: getSessionDefaultTitle(newDateTimestamp, uiLanguage) };
        }
      }
    }
