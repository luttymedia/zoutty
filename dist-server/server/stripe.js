    ...
    try {
        switch (event.type) {
            case 'checkout.session.completed': {
                ...
                const targetTier = session.metadata?.target_tier || 'plus';
                            ...
                            for (const l of logsToRevoke) {
                                if (l.status === 'active' && l.referrer_id) {
                                    const { data: refProfile } = await supabase
                                        ...
                                        .eq('id', l.referrer_id)
                                        .maybeSingle();
                                    if (refProfile && (l.reward_type === 'plus_credit' || l.reward_type === 'student_credit' || l.reward_type === 'teacher_credit')) {
...
export async function updateSubscription(targetTier /* | 'teacher' (disabled) */, authHeader) {
    // TEACHER PLAN (COMMENTED/DISABLED for student-centric pivot; reactivate with new nomenclature if needed)
    if (targetTier === 'teacher') {
    ...
    if (!priceId)
        throw { statusCode: 500, error: "Missing STRIPE_PLUS_PRICE_ID in environment." };
