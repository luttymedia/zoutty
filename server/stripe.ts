...
export async function updateSubscription(targetTier: 'student' | 'teacher' | 'free', authHeader?: string) {
  ...
  const user = await getAuthenticatedUser(authHeader);
  if (!user) throw { statusCode: 401, error: 'Authentication required' };
...
export async function getSubscriptionStatus(authHeader?: string) {
  const stripe = getStripe();
  if (!stripe) {
...
export async function cancelDowngrade(authHeader?: string) {
  const stripe = getStripe();
  if (!stripe) {
    const user = await getAuthenticatedUser(authHeader);
    if (user) {
...
export async function reactivateSubscription(authHeader?: string) {
  const stripe = getStripe();
  if (!stripe) {
    const user = await getAuthenticatedUser(authHeader);
    if (user) {
