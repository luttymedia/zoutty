 ...
 */
export async function processMaturedReferrals(userId: string) {
  const supabase = getSupabaseAdmin();
...
export async function getReferralStats(userId: string) {
  ...
  const pendingRefundCount = logs ? logs.filter((l: any) => l.status === 'pending_refund_period').length : 0;
  const activeBoostsCount = logs ? logs.filter((l: any) => l.status === 'active' && l.reward_type === 'free_boost').length : 0;
  const paidConversionsCount = logs ? logs.filter((l: any) => l.status === 'active' && (l.reward_type === 'student_credit' || l.reward_type === 'teacher_credit')).length : 0;

  const isBoostActive = Boolean(
    profile?.referral_boost_expires_at &&
